...
public class QnADAO {
	...
	public boolean newReply(Reply reply) {
		try {
			...
			String time1 = format1.format(time);
			String sql1 = "insert into reply (rid,qid,rcon,day) values(cnt3.NEXTVAL,?,?,?)";// id는 관리자만 하므로 스크립트릿으로 뺄지
																							// 확인
			...
			while (rs.next()) {// 게시글 모음
				...
				System.out.println(rs.getInt("qstatenum"));
				if (rs.getInt("qstatenum") > 0) {
					String sqla = "update QnA set qstate='답변완료' where qid=?"; // qstatenum이 0보다 크면 답변완료 0이면 접수완료로 바꿔줌
																				// (댓글삭제하거나 댓글을 새로 만들떄마다 확인
					pstmt = conn.prepareStatement(sqla);
		...
		try {
			...
			while (rs.next()) {// 게시글 모음
				...
				if (rs.getInt("qstatenum") > 0) {
					...
					String sqla = "update QnA set qstate='접수완료' where qid=?"; // qstatenum이 0보다 크면 답변완료 0이면 접수완료로 바꿔줌
																				// (댓글삭제하거나 댓글을 새로 만들떄마다 확인)
					pstmt = conn.prepareStatement(sqla);
